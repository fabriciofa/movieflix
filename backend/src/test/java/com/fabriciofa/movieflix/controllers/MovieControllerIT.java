...
import com.fabriciofa.movieflix.tests.TokenUtil;
...
public class MovieControllerIT {
	...
	public void findByGenreShouldReturnOrderedPageWhenVisitorAuthenticated() throws Exception {
		...
		result.andExpect(jsonPath("$.content[0].title").value("A Lenda do Cavaleiro Verde"));
		result.andExpect(jsonPath("$.content[0].subTitle").isEmpty());
		result.andExpect(jsonPath("$.content[0].year").isNotEmpty());
		result.andExpect(jsonPath("$.content[0].imgUrl").isNotEmpty());

		result.andExpect(jsonPath("$.content[1].title").value("Free Guy"));
		result.andExpect(jsonPath("$.content[2].title").value("Bloodshot"));
		result.andExpect(jsonPath("$.content[3].title").value("Kingsman"));
		result.andExpect(jsonPath("$.content[4].title").value("Scary Movie"));
		result.andExpect(jsonPath("$.content[5].title").value("Matrix Resurrections"));
		result.andExpect(jsonPath("$.content[6].title").value("Dune"));
	}
	...
	public void findByGenreShouldReturnFilteredMoviesWhenGenreIsInformed() throws Exception {
		...
		result.andExpect(jsonPath("$.content[1].title").value("Scary Movie"));
	}
}